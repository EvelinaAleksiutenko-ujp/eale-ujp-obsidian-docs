### Correction notes and questions

#### General note:
1. Sometimes the list options ends with different punctuation sign ',' or '.', or sometimes without it al all. There should become consistency in it, just for prettier view.
2. Some of the comments below is about rewriting some sentences to *Note: text* format.
3. Add info about extension SlicerRT for 3D Slicer.
#### Segmentace(why in CZ?)
1. "Our task is focused **only on organs at risk (OAR)**" - should be OAR's
2. Should say about RTS more maybe "RTSTRUCT" what is the form of it, is it a maks or ct with mask, etc?
...
#### Registration
1. "cca"?
2. I am not sure that the rigid registration s hardly reproducible, maybe it is subjective, etc?
3. ''DIR models anatomical changes rather than only position differences.' it looks like some word are missing here
4. ''deformation field' might be better if written "Deformation Vector Field(DVF)" 
...
8. I am not sure that the info in chapter ## **Registration Algorithms (Conceptual Overview)** is enough. We a re taking aout metrcis and regularization but not about algorithm.
...
14. Segmentation maps generated - we are not generating them, we annotate the images, it seems so. Or it shoud be 'Segmentation maps generated. Annotators checks and corrects'
15. if possible generate maps from images directly, else generate synthetic CT (sCT) from them instead - segment sCT - used these segmented maps instead' - if I did not know what is being conveyed I would not understand, add please more explanations
16. 'with problem of deformable registration' - probably better write more concrete that it will help with training DIR itself as we have labels; as current model need to be trained with labels 
...
3. 'patient coordinate system compatibility.' - did not get it
4. 'Currently in development' - should be as note or should be in the name of the chapter.
		Example: *Note*: *Currently in development*
5. '- _Different models may exist for:_
